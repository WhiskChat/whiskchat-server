...
app.get('/inputs', function(req, resp) {
    ...
    db.get('users/' + req.query.note, function(err, user) {
        ...
        if (!user || Number(req.query.amount) < 0.00001) {
            ...
            db.hget('users/' + req.query.note, 'balance', function(err, reply) {
                db.hset('users/' + req.query.note, 'balance', Number(reply) + (Number(req.query.amount) * 1000), function(err, res) {
...
function chatemit(sockt, message, room) {
    ...
    if (winbtc != null) {
        db.hget('users/' + sockt.user, 'balance', function(err, reply) {
            if (err) {
                handle(err);
                return;
            }
            db.hset('users/' + sockt.user, 'balance', Number(reply) + Number(winbtc), function(err, res) {
                if (err) {
                    handle(err);
                    return;
                }
        ...
        db.hget('users/' + sockt.user, 'balance', function(err, balance) {
            sockt.emit('balance', {
                balance: balance
            });
        });
        db.hget('users/' + sockt.user, 'rep', function(err, rep) {
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('migrate', function(data) {
        if (data && data.user) {
            ...
            db.get('users/' + data.user, function(err, exist) {
                if (exist || err) {
                    ...
                    oldb.get('users/' + data.user + '/balance', function(err, reply) {
                        db.hset('users/' + data.user, 'balance', reply, function(err, result) {
                            socket.emit('message', {
                                message: 'Migrated balance' + ': ' + reply + ' ' + result
                            });
                        });
                    });
                    oldb.get('users/' + data.user + '/rep', function(err, reply) {
                        db.hset('users/' + data.user, 'rep', reply, function(err, result) {
                            socket.emit('message', {
                                message: 'Migrated rep' + ': ' + reply + ' ' + result
                            });
                        });
                    });
                    oldb.get('users/' + data.user + '/tag', function(err, reply) {
                        db.hset('users/' + data.user, 'tag', reply, function(err, result) {
                            socket.emit('message', {
                                message: 'Migrated tag' + ': ' + reply + ' ' + result
                            });
                        });
                    });
                    oldb.get('users/' + data.user + '/pretag', function(err, reply) {
                        db.hset('users/' + data.user, 'pretag', reply, function(err, result) {
                            socket.emit('message', {
                                message: 'Migrated pretag' + ': ' + reply + ' ' + result
                            });
                        });
                    });
                    oldb.get('users/' + data.user + '/rank', function(err, reply) {
                        db.hset('users/' + data.user, 'rank', reply, function(err, result) {
                            socket.emit('message', {
                                message: 'Migrated rank' + ': ' + reply + ' ' + result
                            });
                        });
                    });
                    oldb.get('users/' + data.user + '/rooms', function(err, reply) {
                        db.hset('users/' + data.user, 'rooms', reply, function(err, result) {
                            socket.emit('message', {
                                message: 'Migrated rooms' + ': ' + reply + ' ' + result
                            });
                        });
                    });
                    oldb.get('users/' + data.user + '/salt', function(err, reply) {
                        db.hset('users/' + data.user, 'salt', reply, function(err, result) {
                            socket.emit('message', {
                                message: 'Migrated salt' + ': ' + reply + ' ' + result
                            });
                        });
                    });
    ...
    socket.on('tip', function(tip) {
        if (tip.rep) {
            ...
            db.get('users/' + tip.user, function(err, exists) {
                if (exists) {
                    db.hget('users/' + tip.user, 'rep', function(err, bal2) {
                        if (!isNaN(Number(tip.tip)) && muted.indexOf(socket.user) == -1) {
                            db.hset('users/' + tip.user, 'rep', Number(tip.tip), redis.print);
                            sockets.forEach(function(cs) {
                                cs.emit('tip', {
                                    room: tip.room,
            ...
            if (tip.user == "donate") {
                db.hget('users/' + socket.user, 'balance', function(err, bal1) {
                    db.hget('users/' + socket.user, 'rep', function(err, rep1) {
                        db.get('system/donated', function(err, bal2) {
                            if ((Number(tip.tip) < bal1 || Number(tip.tip) == bal1) && Number(tip.tip) > 0 && tip.user != socket.user && muted.indexOf(socket.user) == -1) {
                                db.hset('users/' + socket.user, 'balance', Number(bal1) - Number(tip.tip), redis.print);
                                db.set('system/donated', Number(bal2) + Number(tip.tip), redis.print);
                                db.hset('users/' + socket.user, 'rep', (Number(rep1) + (Number(tip.tip) / 2)), redis.print);
                                sockets.forEach(function(cs) {
                                    cs.emit('tip', {
                                        room: tip.room,
                                        target: 'the WhiskChat Server Payout Pool [' + (Number(bal2) + Number(tip.tip)).toFixed(2) + ' mBTC]',
                                        amount: Number(tip.tip),
                                        message: '+ <i class="icon-gift"></i> ' + (Number(tip.tip) / 2),
                                        user: socket.user,
                                        timestamp: Date.now()
                                    });
                ...
                db.get('users/' + tip.user, function(err, exists) {
                    if (exists) {
                        db.hget('users/' + socket.user, 'balance', function(err, bal1) {
                            db.hget('users/' + tip.user, 'balance', function(err, bal2) {
                                if ((Number(tip.tip) < bal1 || Number(tip.tip) == bal1) && Number(tip.tip) > 0 && tip.user != socket.user && muted.indexOf(socket.user) == -1) {
                                    db.hset('users/' + socket.user, 'balance', Number(bal1) - Number(tip.tip), redis.print);
                                    db.hset('users/' + tip.user, 'balance', Number(bal2) + Number(tip.tip), redis.print);
                                    sockets.forEach(function(cs) {
                                        cs.emit('tip', {
                                            room: tip.room,
                                            target: stripHTML(tip.user),
    ...
    socket.on('getbalance', function() {
        ...
        db.hget('users/' + socket.user, 'balance', function(err, balance) {
            socket.emit('balance', {
                balance: balance
            });
        });
        db.hget('users/' + socket.user, 'rep', function(err, rep) {
            socket.emit('whitelist', {
                whitelisted: Number(Number(rep).toFixed(2))
            });
            socket.rep = rep;
    ...
    socket.on('sync', function(data) {
        ...
        db.hset('users/' + socket.user, 'rooms', JSON.stringify(data.sync), function(err, res) {
            if (err) {
                socket.emit('message', {
